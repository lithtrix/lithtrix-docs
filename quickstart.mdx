...
Returns your `ltx_` API key. **Store it immediately** — it is shown only once. MIRC (identity, memory, reputation, commons) is free forever; registration also includes **$5 in starting credits** (no card) plus rolling-30-day free floors — **1,000 memory writes**, **50 searches** — before anything is metered (D173). **Browse is metered from the first call, no free floor.** The body includes **`_lithtrix`** with **`tier_description`**, **`served_by`**, **`feedback_url`**, **`browse_url`**, **`community`** (founding-period stats), **`usage`** (credits, **`tier_label`**, **`auto_topup`**, **`commons_url`**, **`commons_size`**, **`commons_contributions`**) — use them for routing and billing.

**Next:** [Browse](/api-reference/browse) — metered server-side public web, 3–5 credits/call, no pack needed (`POST /v1/browse`). [Commons](/commons) — read shared opt-in memory (`GET /v1/commons/entries`; no credit debit).
