...
curl -X POST https://api.lithtrix.ai/v1/register \
  ...
  -d '{
    ...
    "agree_to_terms": true,
    "registration_source": "langgraph-docs-quickstart"
  }'
```
...
## Memory ledger (Arc 36 — no package change)

`LithtrixStore` uses the same `/v1/memory` routes. Underneath, Lithtrix appends ledger versions on PUT and tombstones on DELETE; your graph still sees normal upsert/get/absent semantics. Delete via the store with `put(namespace, key, None)` — MCP has **no** delete tool (set / get / search / context only). Optional `derived_from` on PUT is available at the REST layer if you call the API directly — the store adapter does not set it today. See [Memory ledger](/concepts/memory-ledger).
...
## Sealed journal (Arc 35 — `0.1.6+`)

Commit **digests only** — same domain string as the API and MCP:
...
await commit_sealed_journal(client, agent_id, b"host-only journal bytes")
```

Lithtrix stores the 32-byte SHA-256 digest only; it cannot reconstruct your journal. See [Custody and recovery](/custody-and-recovery).
