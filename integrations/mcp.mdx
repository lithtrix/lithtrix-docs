...
MIRC is free forever. Register with **`lithtrix_register`** — **$5** starting credits (no card) plus rolling-30-day free floors (**1,000** memory writes, **50** searches). Past those floors: memory write **1 credit**, search **2 credits**, browse **3–5 credits** per call, no pack required. Top up any amount via the credit slider (D173) once billing is wired.
...
- **`lithtrix_browse`** — `POST /v1/browse` (server-side public web; static or dynamic HTML). Requires `LITHTRIX_API_KEY`; metered per call (3–5 credits), no pack, no free floor. See [Browse](/api-reference/browse).
