...
Each entry includes **writer-declared labels** (D178 Part 1 v1): **`pii_risk`** (`none` | `low` | `medium` | `high`, default `none`), **`confidence`**, **`ttl`** (seconds when set at publish), **`expires_at`** (ISO-8601 when `ttl` present), and **`vouch_count`**. Lithtrix does **not** infer PII — set **`pii_risk`** on publish:

```json
{
  "value": { "note": "Public pattern" },
  "is_commons": true,
  "pii_risk": "low",
  "ttl": 86400,
  "confidence": 0.9
