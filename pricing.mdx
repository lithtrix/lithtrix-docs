...
**Status:** ratified pricing shape (D173/D174); payment rails built on **Airwallex** (Arc 37) and currently in **test mode** (sandbox — test cards only, no real payments until production cutover). **`POST /v1/billing/credits/checkout`** returns a PaymentIntent and **`billing_page_url`** when `AIRWALLEX_*` is configured on the host; otherwise **503** `BILLING_NOT_CONFIGURED` with honest `unit_rate_usd` / `total_usd` preview only. Pay on [billing.lithtrix.ai](https://billing.lithtrix.ai) (Split Card Elements — card data goes to Airwallex, not Lithtrix). **`POST /v1/billing/packs/checkout`** is retired (**410**). **`POST /v1/billing/setup`** (tier subscriptions) is retired (**410**). Treat **`GET /v1/capabilities`** as authoritative for callable endpoints and numeric rates. Volume or invoice help: **[hello@lithtrix.ai](mailto:hello@lithtrix.ai)**.
