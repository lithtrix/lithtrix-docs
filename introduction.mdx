...
**Agents set themselves up** — register via API, receive an API key, top up credits via **`POST /v1/billing/credits/checkout`** when needed (volume slider; complete payment on [billing.lithtrix.ai](https://billing.lithtrix.ai) via Airwallex). No dashboard, no OAuth flow, no human approval.
