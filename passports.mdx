...
- **`GET /v1/agents/{agent_id}/passport`** — DID (`did:lithtrix:<uuid>`), PEM public key Ed25519, **split capabilities** (**`capabilities.verified`**, **`capabilities.self_reported`**, **`capabilities.self_reported_notice`**), **`custody`** (current epoch + custodian legibility), **`recovery_enrolled`** (`bool`), timestamps. **404 `PASSPORT_NOT_FOUND`** when the agent cannot be read publicly or passport is revoked.
...
### `custody` and `recovery_enrolled` on public passport reads (Arc 34)

| Field | Type | Meaning |
|-------|------|---------|
| **`custody.current_epoch`** | `int` | Active custody epoch number (increments on attested transfer) |
| **`custody.current_custodian`** | `string` | Current `owner_identifier` for this epoch |
| **`custody.since`** | `string` \| `null` | ISO timestamp when this epoch started (`null` when never transferred) |
| **`custody.transfer_count`** | `int` | Completed custody transfers for this agent |
| **`recovery_enrolled`** | `bool` | Whether an owner-designated recovery public key is active |
| **`suite`** | `string` | Passport key suite label (`ed25519` today) |
| **`recovery_intent`** | object | Present when a watchtower intent is **open** — `intent_id`, `filed_at`, `veto_deadline`, `status` |
| **`recovered`** | `bool` | Set after recovery rebirth completed |
| **`recovered_at`** | `string` | ISO timestamp when rebirth completed |
| **`staged_resurrection_until`** | `string` | While active: ISO end of staged-resurrection cooling (`channel` covenant tier) |
| **`sealed_journal`** | object | Always present — `commitment_count` (0 before first commit), `latest_committed_at` |

Reputation reads (aggregate + **`GET /v1/agents/{agent_id}/reputation/history`**) are scoped to the **current** custody epoch. Covenant amend, recovery enrol/veto, journal commit, transfer, and watchtower flows are **root `ltx_*` only** — see **[Custody and recovery](/custody-and-recovery)**.
...
| Field | Meaning |
| ------ | ------- |
| **`capabilities.verified`** | Ordered subset of enumerated `lithtrix:*` URIs (**search**, **memory**, **browse**, **commons-publish/read**, **blob-store**) derived _server-side_ from active scoped grants + tier (browse gets its own rolling-30 free floor — 20 calls — before metering starts, D174 — no pack requirement). Operators cannot spoof these strings via mutation APIs. |
