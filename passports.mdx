...
Reputation reads (aggregate + **`GET /v1/agents/{agent_id}/reputation/history`**) are scoped to the **current** custody epoch. Transfer flows, recovery enrol, and lost-root-key recovery are **root `ltx_*` only** — see **[Custody and recovery](/custody-and-recovery)**.
