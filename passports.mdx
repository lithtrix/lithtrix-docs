...
| Field | Meaning |
| ------ | ------- |
| **`capabilities.verified`** | Ordered subset of enumerated `lithtrix:*` URIs (**search**, **memory**, **browse**, **commons-publish/read**, **blob-store**) derived _server-side_ from active scoped grants + tier (browse is metered per call from the first call, D173 — no pack requirement). Operators cannot spoof these strings via mutation APIs. |
